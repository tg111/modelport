...
* 在后台添加 OpenAI-compatible 渠道：API 地址、密钥、备注、服务商链接。
* 每个渠道可选择 `Responses 原生` 或 `Chat Completions 兼容` 协议。
...
如果某个渠道只支持 `/v1/chat/completions`，请把渠道协议设为 `Chat Completions 兼容`。此时 Codex 请求 `/v1/responses` 会被桥接到上游 `/v1/chat/completions`。
